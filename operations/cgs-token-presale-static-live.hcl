job "cgs-token-presale-static-live" {
  datacenters = [ "mb-hel" ]
  type = "batch"

  reschedule { attempts = 0 }

  group "cgs-token-presale-static-live-group" {
    count = 1

    task "cgs-token-presale-static-live-task" {
      driver = "docker"

      config {
        image = "ghcr.io/communitygrocerystore/token-presale:live"
        force_pull = true
      ...
      env {
        PHASE = "live"
        DEPLOY_BUCKET = "token-presale-live-communitygrocerystore-com"
